...
class ReadFile:
    ...
    def readJsonFile(self,file_name_path):
        ...
        dict_data['item'].append({
            'text': json_data['text'],
            'screename': json_data['user']['screen_name'],
    ...
    def sendRestRequest(self, dict_data):
        ...
        for item in dict_data['item']:
            dict_file_values['item'] = {'text': item['text'], 'screename': item['screename'], 'location': item['location'], 'description':item['description']}
            text = tag_remover.strip_tags(item['text'])
